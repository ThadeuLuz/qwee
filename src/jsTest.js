...
function begin() {
  ...
  if (list.length < 1) {
    console.log(chalk.red("Could not find a controller!"));
    process.exit();
  }
  ...
  function rumbleScript(chg, g) {
    ...
    //Rumble Off:
    if (
      (chg.l2 || chg.r2 || chg.l3 || chg.start) &&
      !(g.analog.l2 || g.analog.r2 || g.digital.l3 || g.digital.start)
    ) {
      g.rumble(0, 0);
      console.log("rumble off");
    }
    //Change LED Pattern:
    if (chg.ps && g.digital.ps) {
      g.setLed(nLedVal);
      console.log("led set " + nLedVal);
      nLedVal++;
